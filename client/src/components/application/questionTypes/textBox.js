  ...
  return (
    <div className="questionContainer">
      ...
      <a href={`#${(nav)-1}`} className="navlink">
        <div className="prevBtn" tabIndex={-1} onClick={() => handleNextPrevNav(index, 'prev')}><ArrowLeftRoundIcon /></div></a>
        <a href={`#${(nav)+1}`} className="navlink"><div className="nextBtn" tabIndex={-1} onClick={() => handleNextPrevNav(index, 'next')}><ArrowLeftRoundIcon /></div></a>
      </div>
    </div>
  )
}