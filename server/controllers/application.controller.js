...
function getAllQuestions(req,res){
    ...
    (async() => {
        ...
        if(getRes === 500){
            ...
            getRes.map(o => { o.question_opt = JSON.parse(o.question_opt);});
...
function getNestedQuestion(req,res){
    ...
    (async ()=> {
        const quesRes = await seq.seqFindAll(Question, ['id', 'category_id', "question", "question_type_id", "question_opt", "status"], {parent_id:parent_id}, {model:Category, attributes:['id','name']});
        if(quesRes === 500){
...
function submitApplication(req,res,type){
    ...
    (async () => {
        ...
        for(let question_id in inputs){
            ...
            const quesRes = await seq.seqCreate(AppQuestion,{app_id:app_id, question_id:question_id});
            ...
            if(Array.isArray(answer)){
                answer.forEach(async(arrayItem) => {
                    ...
                    const ansRes = await seq.seqCreate(Answer, {answer:arrayItem, question_id:question_id, app_question_id:app_question_id});
                ...
                const ansRes = await seq.seqCreate(Answer, {answer:answer, question_id:question_id, app_question_id:app_question_id});
                if(ansRes === 500){
                    isError = true;
                ...
                //console.log("yes");
                const ansRes = await seq.seqCreate(Answer, {answer:JSON.stringify(answer), question_id:question_id, app_question_id:app_question_id});
                if(ansRes === 500){
                    isError = true;
...
module.exports = {
    getAllQuestions, getCountryNames, getNestedQuestion, submitApplication, draftApplication
}